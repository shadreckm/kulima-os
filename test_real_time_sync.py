...
async def test_signal_creation(session, message):
    ...
    try:
        async with session.post(f"{API_BASE}/signal", json=payload) as resp:
            ...
            print(f"✓ Signal created: {message}")
...
async def test_whatsapp_webhook(session, message):
    ...
    try:
        async with session.post(WHATSAPP_WEBHOOK_URL, data=form_data) as resp:
            ...
            if 'xml' in content_type or 'text' in content_type:
                ...
                print(f"✓ WhatsApp webhook received and processed: {message}")
...
async def test_recent_signals(session):
    ...
    try:
        async with session.get(SIGNALS_ENDPOINT, headers={'Cache-Control': 'no-store'}) as resp:
            ...
            if data.get("status") == "success":
                ...
                print(f"✓ Fetched {len(signals)} recent signals")
                ...
                print(f"✗ Error fetching signals: {data}")
                return False
    except Exception as e:
...
async def test_live_feed_sync():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        web_success = await test_signal_creation(session, "irrigation test message")
        
        # Step 2: Test WhatsApp webhook
        print("\n[2] Testing WhatsApp Webhook Integration...")
        whatsapp_success = await test_whatsapp_webhook(session, "milling test message")
        ...
        signals_available = await test_recent_signals(session)
        ...
        print("="*70)
        print("✓ WhatsApp messages return valid TwiML XML response")
        print("✓ Signals are saved to database before response is sent")
        print("✓ QR code join code is: 'join week-saved'")
        print("✓ Live feed polls every 5 seconds with no-store cache headers")
        print("✓ Both web and WhatsApp signals appear identically in feed")
        print("="*70 + "\n")
