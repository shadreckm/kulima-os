...
KNOWN_LOCATIONS = [
    'luwinga', 'mchesi', 'chengani', 'chatinkha', 'area 25', 'area 26',
    'area 27', 'area 16', 'area 17', 'area 18', 'area 19', 'area 20',
    'kampanja', 'bingu', 'boma', 'tengani', 'nkhoma', 'zomba', 'blantyre',
    'lilongwe', 'mzuzu', 'karonga', 'kasungu', 'dowa'
]
...
def build_cluster_summary(signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for signal in signals:
        ...
        location = signal.get('location') or DEFAULT_LOCATION
        cluster_id = signal.get('cluster_id') or f"{zone}-{location.replace(' ', '_')}"
        ...
        if key not in clusters:
            clusters[key] = {
                'cluster_id': key,
                'cluster_name': f"{location} Cluster" if location and location != DEFAULT_LOCATION else f"{zone} Local Cluster",
                'zone': zone,
                'location': location,
                'activities': {},
                'crops': set(),
